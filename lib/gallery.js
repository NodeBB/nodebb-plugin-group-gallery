	...
	if (!params || !params.group || !params.url || !params.uid) {
		...
		NodeBB.db.incrObjectField('group-gallery', 'nextImageId', function(err, nextImageId) {
			...
			async.parallel([
				...
				function(next) {
					NodeBB.db.incrObjectField('group-gallery:group:' + groupSlug, 'imagecount', next);
				}
			], function(err, result) {
...
Gallery.getGroupImageCount = function(group, callback) {
	if (!group) {
		...
		NodeBB.db.getObjectField('group-gallery:group:' + group, 'imagecount', function(err, result) {
...
function getImages(ids, callback) {
	...
	}), userFields = ['uid', 'username', 'userslug'], uids, userData;
