	...
	if (!params || !params.group || !params.url || !params.uid) {
		...
		db.incrObjectField('group-gallery', 'nextImageId', function(err, nextImageId) {
			...
			async.parallel([
				...
				function(next) {
					db.sortedSetAdd('group-gallery:group:' + groupSlug + ':images', image.timestamp, nextImageId, next);
				},
				function(next) {
					db.incrObjectField('group-gallery:group:' + groupSlug, 'imagecount', next);
				}
			], function(err, result) {
	...
	async.parallel({
		...
		if (parseInt(result.image.uid, 10) === parseInt(uid, 10) || result.isAdmin) {
			async.parallel([
				...
				function(next) {
					db.sortedSetRemove('group-gallery:group:' + result.image.group + ':images', imageId, next);
				},
				function(next) {
					db.decrObjectField('group-gallery:group:' + result.image.group, 'imagecount', next);
				}
			], callback);
		}
	});
...
Gallery.getImagesByGroupName = function(group, start, end, callback) {
	if (!group) {
		...
		db.getSortedSetRevRange('group-gallery:group:' + group + ':images',
