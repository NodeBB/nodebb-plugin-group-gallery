...
Gallery.getGroupImageCountByUid = function(group, uid, callback) {
	if (!group || !uid || isNaN(parseInt(uid, 10))) {
		callback(new Error('invalid-data'), []);
	} else {
		group = NodeBB.utils.slugify(group);

		NodeBB.db.getSortedSetRange('group-gallery:group:' + group + ':images', 0, -1, function(err, result) {
			async.filter(result, function(image, next) {
				next(parseInt(image.uid, 10) === parseInt(uid, 10));
			}, function(results) {
...
Gallery.getImagesByUid = function(group, uid, start, end, callback) {
	if (!group || !uid || isNaN(parseInt(uid, 10))) {
		callback(new Error('invalid-data'), []);
	} else {
		group = NodeBB.utils.slugify(group);
		...
		NodeBB.db.getSortedSetRevRange('group-gallery:group:' + group + ':images',
			isNaN(start) ? 0 : start, -1, function(err, ids) {
				if (err) {
					callback(err, []);
				} else {
					getImages(ids, function(err, images) {
						async.filter(images, function(image, next) {
							next(parseInt(image.uid, 10) === uid);
						}, function(results) {
							callback(null, results.splice(0, end + 1));
						});
					});
				}
			}
